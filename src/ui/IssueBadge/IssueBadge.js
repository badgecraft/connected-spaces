...
const toErrorList = errors => Object
    .keys(errors || {})
    .map(key => errors[key])
    .filter(item => item)
    .map(item => typeof item === 'object' ? toErrorList(item) : item)
    .reduce((prev, curr) => (curr instanceof Array ? [...prev, ...curr] : [...prev, curr]), []);
...
const InviteStatus = ({ status }) => {
    ...
    if (status.ok) {
        return (<Status>{t`Issued.`}</Status>);
    }

    const errors = translateErrors({
        errors:       status.errors,
        translations: {
            alreadyHasBadge: () => t`Already has this badge`,
            depsNotMet:      () => t`User does not meet badge dependencies yet`,
        },
    });

    return (<Status error>{toErrorList(errors).join(', ')}</Status>);
};
...
const IssueBadge = ({ value, onChange, inviteStatus, disabled }) => (
    <Root>
        <Search>
            <UserSelect
                onChange={user => onChange(addValue(value || [], user))}
                value={null}
                escapeClearsValue
                exclude={value || []}
                disabled={disabled}
